...
class PerceptronClassifier:
    ...
    def plot_pixels(self):
        # 点を画面に描画
        for p,type in zip(self.x,self.t):
            print(p,":",type)
            if type == 1:
                plt.plot(p[0],p[1],"o",color="b") # 1は青い○
            else:
        ...
        while updated:
            ...
            for category, features in zip(self.t, self.x):
                ...
                if predict != category:
                    ...
                    self.weight = self.weight + self.alpha * t * np.append(features, 1)                                
                    updated = True
                    n = n + 1
        # 確定した線を描画する
    ...
    def plot_line(self,n,last):
         ...
         if self.weight[1] != 0:
             x = np.arange(-0.1,1.1,0.1)  # xの値域(0, 1, 2, 3)
